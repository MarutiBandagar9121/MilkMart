...
<section class="dashboard">
   ...
   <div class="box-container">
      ...
      <div class="box">
         <?php
            $total_pendings = 0;
            $select_pending = pg_query($conn, "SELECT total_price FROM orders WHERE payment_status = 'pending'") or die('query failed');
            if(pg_num_rows($select_pending) > 0){
               while($fetch_pendings = pg_fetch_assoc($select_pending)){
      ...
      <div class="box">
         <?php
            $total_completed = 0;
            $select_completed = pg_query($conn, "SELECT total_price FROM orders WHERE payment_status = 'completed'") or die('query failed');
            if(pg_num_rows($select_completed) > 0){
      ...
      <div class="box">
         <?php 
            $select_admins = pg_query($conn, "SELECT * FROM users WHERE user_type = 'admin'") or die('query failed');
            $number_of_admins = pg_num_rows($select_admins);
         ?>
